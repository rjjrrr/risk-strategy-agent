...
class MockProvider(LLMProvider):
    def _response(self,messages,model='mock-v1',**kwargs):
        ...
        if 'AGENT_TYPE=ANALYSIS_AGENT' in joined:
            return json.dumps({'analysis_summary':'NEW-only deterministic context reviewed.','semantic_findings':[{'title':'Available rule evidence','finding_type':'RULE_SIGNAL','evidence':{'scope':'NEW'},'interpretation':'Rule evidence can support a reviewable hypothesis.','confidence':'HIGH','source_ids':[]}],'hypotheses':[{'title':'Recent activity intensity may indicate higher risk','risk_mechanism':'Concentrated recent activity may indicate credit-seeking pressure.','evidence':{'scope':'NEW'},'source_fields':['query_cnt_7d'],'expected_direction':'HIGHER_RISK_WHEN_HIGH','confidence':'MEDIUM','estimated_cost':'LOW'}],'feature_proposals':[{'feature_name':'query_cnt_7d_review','feature_type':'RAW','source_fields':['query_cnt_7d'],'formula':'query_cnt_7d','semantic_meaning':'Recent query intensity pending governance validation.','expected_direction':'HIGHER_RISK_WHEN_HIGH','evidence':{'scope':'NEW'},'confidence':'MEDIUM','status':'REVIEW','desired_operations':[],'required_data_sources':['CURRENT_WIDE_TABLE'],'entity_key':None,'time_window':None,'application_time_field':None,'feature_engine_requirements':{}}],'warnings':[],'missing_information':['Experiment history may be unavailable.']},ensure_ascii=False)
        if 'AGENT_TYPE=DECISION_AGENT' in joined:
            return json.dumps({'diagnosis':'NO_ACTION_REQUIRED','diagnosis_confidence':'LOW','evidence':[],'candidate_actions':[{'action_type':'NO_ACTION','reason':'Mock runtime cannot authorize a real experiment','risk':'LOW','cost':'LOW','evidence_confidence':'LOW','novelty':'UNKNOWN','credit_direction':'UNKNOWN','priority':0,'requires_human_approval':False}],'selected_action':{'action_type':'NO_ACTION','reason':'Mock runtime cannot authorize a real experiment','risk':'LOW','cost':'LOW','evidence_confidence':'LOW','novelty':'UNKNOWN','credit_direction':'UNKNOWN','priority':0,'requires_human_approval':False},'expected_effect':'No model or experiment state change.','risk_level':'LOW','requires_human_approval':False,'stop_reason':'MOCK_RUNTIME_READ_ONLY','missing_information':['REAL_LLM_RUNTIME']},ensure_ascii=False)
...
class OpenAICompatibleProvider(LLMProvider):
    ...
    def chat(self,messages,model,temperature=.2,max_tokens=1200,response_format=None,**kwargs):
        ...
        try:
            ...
            return {'content':choice['message']['content'],'usage':data.get('usage',{}),'model':data.get('model',model),'finish_reason':choice.get('finish_reason'),'execution_mode':'LLM'}

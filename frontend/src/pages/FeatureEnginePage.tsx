...
export default function FeatureEnginePage({datasetId,onError}:Props){
  ...
  return <div className="feature-engine-page">
    ...
    <div className="metrics compact">
      <div className="metric-card"><span>特征建议</span><b>{specs.length}</b><small>已完成结构化</small></div>
    ...
    {capabilities&&<section className="card engine-capabilities"><h3>可用计算能力</h3><p><b>计算算子</b> {capabilities.operators.join(', ')}</p><p><b>时间窗口</b> {capabilities.windows.join(', ')}</p><p><b>暂不支持示例</b> {capabilities.unsupported_examples.join(', ')}</p></section>}
